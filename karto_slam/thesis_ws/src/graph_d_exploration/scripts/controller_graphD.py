...
from graph_d_exploration.msg import PointArray, InfoMatrix
...
def node():
    global frontiers_, merged_frontiers_, map_data_, robot_, rec_centroids_flag, rec_goal_flag, goal_coordinates, arrAck, robot_name
    if SHOW_DEBUG_PATH_:
    ...
    while not rospy.is_shutdown():
        ...
        # Print log
        rospy.loginfo(f'{robot_name} waiting for all agents to reach their goal')
        ...
        while not all(arrAck):
            ...
            arrAck[int(robot_name.split("_")[1])] = True
            msg = Bool()
            msg.data = True
        ...
        rospy.logwarn(f'{robot_name} published list for being merged')
        ...
        # Print log
        rospy.loginfo(f'{robot_name} received merged frontiers')
        ...
        if robot_.getState() == 1:
            ...
            if goal_coordinates != []:
                ...
                # Access the goal position depending on the i in robot name
                # robot-i with i in [0, ..., n]
                selected_point = goal_coordinates[int(robot_name.split("_")[1])]

                # Fill the arrat for the goal coordinates
                _goal_ = []
                _goal_.append(selected_point.x)
                _goal_.append(selected_point.y)
                num_of_goals +=1 # no of goals sent to eech agent 

...
namespace frontier_exploration {
...

    {
        ...
        _goalPub = _pnh->advertise<geometry_msgs::Point>("goals_topic", 10);
        _goaltimePub = _pnh->advertise<std_msgs::Time>("time_topic", 10);
    ...
    void Explore::makePlan()
    {
        ...
        */
        ...
        point.x = goal.target_pose.pose.position.x;
        point.y = goal.target_pose.pose.position.y;

        // Publish the point message
         _goalPub.publish(point);
        // Create the time message
        std_msgs::Time time_msg;
        time_msg.data = ros::Time::now();  
    // Publish the time message
       _goaltimePub.publish(time_msg);               

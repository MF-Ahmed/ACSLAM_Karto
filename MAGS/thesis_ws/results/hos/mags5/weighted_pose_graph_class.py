...
class weighted_pose_graph:
    ...
    def plot_graph_rot(self, label='Data', color='Blue', draw_l=False, rotate_90_cw=False):
        if self.graph is not None:
            nodes = self.graph.nodes.data('pose')
            ...
            for el in nodes:
                x, y = el[1][0], el[1][1]
                if rotate_90_cw:
                    poses_x.append(y)      # x' = y
                    poses_y.append(-x)     # y' = -x
                else:
                    poses_x.append(x)
                    poses_y.append(y)
            ...
            if draw_l:
                start_x, start_y = poses_x[0], poses_y[0]
                end_x, end_y = poses_x[-1], poses_y[-1]
                plt.plot(start_x, start_y, '*', label='Start', color='black', alpha=1, markersize=15)
                plt.plot(end_x, end_y, 'd', label='End', color='blue', alpha=1, markersize=12)

                label_added = False
                edges_LC = self.get_LC_edges()
                for (u, v) in edges_LC:
                    x1, y1 = self.graph.nodes[u]['pose'][:2]
                    x2, y2 = self.graph.nodes[v]['pose'][:2]
                    if rotate_90_cw:
                        poses_LC_x = [y1, y2]
                        poses_LC_y = [-x1, -x2]
                    else:

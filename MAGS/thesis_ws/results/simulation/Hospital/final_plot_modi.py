...
data_files = sorted([file for file in os.listdir(data_directory) if file.startswith('sim')], key=lambda x: int(x.split("_")[1]))
...
for file in data_files:
    ...
    #if file == 'sim_1_2_robots-AGS.npy' :
        #merged_x = merged_x[0:400]
        #merged_y = merged_y[0:400]
    #elif file == 'sim_3_3_robots-AGS.npy':
        #merged_x = merged_x[0:380]
        #merged_y = merged_y[0:380]
...
for i in range(num_simulations):
    ...
    plt.plot(np.array(merged_x_list[i]) - merged_x_list[i][0], (merged_y_list[i] / generated_map_area_list[i]) * 100,
             label=f'S{_simulation_number} - {num_robots_list[i]}R {method_list[i]}',linewidth=3)
...
for i, num_robots in enumerate(num_robots_list):
    ...
    x_labels.append(f'{simulation_label}({num_robots}R) {_method[0:3]}')

...
data_files = sorted([file for file in os.listdir(data_directory) if file.startswith('exp')], key=lambda x: int(x.split("_")[1]))
...
for file in data_files:
    ...
    if file == 'exp_1_2_robots-AGS.npy':
        merged_x = merged_x[0:16]
        merged_y = merged_y[0:16]
...
for i in range(num_simulations):
    ...
    plt.plot(np.array(merged_x_list[i]) - merged_x_list[i][0], ((merged_y_list[i] / generated_map_area_list[i])*2) * 100,
             label=f'Exp{_simulation_number} - {num_robots_list[i]}R {method_list[i]}',linewidth=3)
...
plt.xlabel('# Agents, approach, Frontier Management (F.M)')
